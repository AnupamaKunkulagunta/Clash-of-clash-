...
def printMap(V):
    ...
    for hl in healers:
        ...
        map_matrix[a+1][b] = hlColor(hl) + Fore.BLACK + 'AL'
        map_matrix[a][b+1] = hlColor(hl) + Fore.BLACK + 'ER'
        map_matrix[a+1][b+1] = hlColor(hl) + Fore.BLACK + 'S '

    for arch in s_archers:
        a= 2*arch.position[0]
        b = 2*arch.position[1]
        map_matrix[a][b] = s_archColor(arch) + Fore.BLACK + 'S'
        map_matrix[a+1][b] = s_archColor(arch) + Fore.BLACK + '  '
        map_matrix[a][b+1] = s_archColor(arch) + Fore.BLACK + 'CH'
        map_matrix[a+1][b+1] = s_archColor(arch) + Fore.BLACK + 'ER'
