...
class Barbarian:
    ...
    def break_wall(self, x, y, V,King):
        ...
        if(target.level<3):
            ...
            if target.health <= 0:
                troops = barbarians+ archers + s_archers  + dragons + balloons + healers + kings
                for troop in troops:
                    r = abs(troop.position[0]-x)
                    c = abs(troop.position[1]-y)
                    if(r<=target.radius and c<=target.radius):
                        troop.deal_damage(target.damage)
                target.destroy()
...
class Stealth_Archer:
    ...
    def invis_time(self):
        time=threading.Timer(self.time, self.isVisible)
        time.start()
...
class Healer:
    ...
    def move(self, troop, King):
        ...
        if(r*r+c*c<=self.range*self.range):
            self.heals(troop.position[0], troop.position[1], King)
    ...
    def heals(self, x, y, King):
            r = abs(x - King.position[0])
            c = abs(y - King.position[1])
            if(r**2 + c**2 <= self.heal_radius**2):
                self.heal_target(King, King)

            troops = barbarians + archers + dragons + s_archers
            for troop in troops:
    ...
    def heal_target(self, troop, King):
            i = troop.position[0] - 1
            j = troop.position[1] - 1
            kings = []
            kings.append(King)
            troops = barbarians+ archers + dragons + balloons + s_archers + kings
...
def move_s_archers(V,type, King):
    if(type == 1):
...
def move_healers(King):
    for heal in healers:
        if(heal.alive == False):
            continue
        closest_troop = search_for_closest_troop(heal.position, King)
        if(closest_troop == None):
            continue
        heal.move(closest_troop,King)
...
def search_for_closest_troop(pos, King):
    ...
    # if(dist < closest_dist and King.health < King.max_health):
    #     flag = 1
    #     closest_dist = dist
    #     closest_troop = King.position
    kings=[]
    kings.append(King)
    troops = barbarians + archers + s_archers + balloons + dragons + kings
    for troop in troops:
